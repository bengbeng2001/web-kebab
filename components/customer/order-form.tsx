...
export function OrderForm() {
  ...
  useEffect(() => {
    const getUserData = async () => {
      ...
      if (user) {
        // Fetch user details from users_public using auth_id
        const { data: users, error: userDetailsError } = await supabase
          .from('users_public')
          .select('id, username, address, phone')
          .eq('auth_id', user.id)
          .single();
        ...
        if (users) {
          setNewCustomerName(users.username || '');
          setNewCustomerAddress(users.address || '');
          setNewCustomerPhone(users.phone || '');
        }
      }
    };
    getUserData();
  ...
  const handleCreate = async () => {
    try {
      ...
      // Get users_public ID using auth_id
      ...
      // Create order using hook with public_users_id
      const orderResult = await createOrder({
        order_number: Math.floor(Math.random() * 1000000),
        public_users_id: userData.id as UUID,
  ...
  if (productsLoading || ordersLoading) return <SkeletonCard/>;
